...
const App: React.FC = () => {
  ...
  const navItems: { label: string; id: Page }[] = [
    { label: 'Products', id: 'QVAL' }, // Simplified for demo
  ...
  const renderPage = () => {
    switch (activePage) {
      case 'HOME': return <LandingPage onNavigate={setActivePage} />;
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-quantum-900/90 backdrop-blur-md border-b border-white/5 py-4' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center gap-8">
                {activePage === 'HOME' ? (
                   navItems.map(item => (
                       <button key={item.id} onClick={() => setActivePage(item.id)} className="text-sm font-medium text-gray-400 hover:text-white transition-colors">{item.label}</button>
                   ))
                ) : (
                    <>
                        <button onClick={() => setActivePage('HOME')} className="text-sm font-medium text-gray-400 hover:text-white transition-colors">Home</button>
                        <button onClick={() => setActivePage('QVAL')} className={`text-sm font-medium hover:text-white transition-colors ${activePage === 'QVAL' ? 'text-white' : 'text-gray-400'}`}>Q-Val</button>
                        <button onClick={() => setActivePage('SUKUK')} className={`text-sm font-medium hover:text-white transition-colors ${activePage === 'SUKUK' ? 'text-white' : 'text-gray-400'}`}>SukukGuard</button>
                        <button onClick={() => setActivePage('REGUSIM')} className={`text-sm font-medium hover:text-white transition-colors ${activePage === 'REGUSIM' ? 'text-white' : 'text-gray-400'}`}>ReguSim</button>
                        <button onClick={() => setActivePage('WAD')} className={`text-sm font-medium hover:text-white transition-colors ${activePage === 'WAD' ? 'text-white' : 'text-gray-400'}`}>Wa'dWizard</button>
                    </>
                )}
            </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            <div>
                ...
                <ul className="space-y-3 text-sm text-gray-500">
                    <li className="hover:text-quantum-500 cursor-pointer" onClick={() => setActivePage('SUKUK')}>SukukGuard</li>
                    <li className="hover:text-quantum-500 cursor-pointer" onClick={() => setActivePage('WAD')}>Wa'dWizard</li>
                    <li className="hover:text-quantum-500 cursor-pointer" onClick={() => setActivePage('QVAL')}>Q-Val</li>
                    <li className="hover:text-quantum-500 cursor-pointer" onClick={() => setActivePage('REGUSIM')}>ReguSim</li>
                </ul>
            </div>
